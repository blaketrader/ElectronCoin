...
{
    QString ret = QString("Electron:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ELT unit input in lnReqAmount, we generate the URI with ELT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ELT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
