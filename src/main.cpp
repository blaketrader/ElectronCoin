...
// Copyright (C) 2014-2015 The Electron developers
...
uint256 hashGenesisBlock("0x00000063044a9b6cad9015b289de8a86a654d5a916fcaaeeefb5b9408448df40");
static const unsigned int timeGenesisBlock = 1400888888;
...
const string strMessageMagic = "Electron Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 ELT if made of 50 ELT inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
	...
	int64 nSubsidy = 20 * COIN;  // First year
    if (nHeight > 525600)
    {
	nSubsidy = 10 * COIN;
	}
...
*/
        ...
        block.nNonce   = 159133830;

        if (fTestNet)
        {
            block.nTime    = 1400088888;
            block.nNonce   = 0;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x6a1105401133793ebc124b837824144b64058bd0b89408cd17201012972f3ec8"));
...
unsigned char pchMessageStart[4] = { 0xf9, 0xbc, 0xac, 0xc4 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ElectronMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ElectronMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("ElectronMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("electron-miner");
    ...
    try { loop {
        ...
        printf("Running ElectronMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("ElectronMiner terminated\n");
        throw;
    }
}
